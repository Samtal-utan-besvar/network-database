...
// Verify reset code with wrong data format
it('User A verify reset code (non-JSON)', (done) => {
    var userRequest = new ArrayBuffer(8);

    testverifyResetPassword(userA.token, userRequest, 'Request is Missing Field: email', done);
});

// Verify reset code with empty field
it('User A verify reset code (Empty Field)', (done) => {
    var userRequest = {
        "email": "",
...
// Verify reset code with missing field
it('User A verify reset code (Missing Field)', (done) => {
    var userRequest = {
    }
...
// Verify reset code with to many fields
it('User A verify reset code (To Many Fields)', (done) => {
    var userRequest = {
...
// Verify reset code with wrong field name
it('User A verify reset code (Wrong Field Name)', (done) => {
    var userRequest = {
...
// Verify reset code with wrong email format
it('User A verify reset code (Wrong Name Format)', (done) => {
    const email = "123ABC"
    var userRequest = {
        "email": email,
        "verify_code": "randomString"
...
// Verify reset code with SQL injection
it('User A verify reset code (SQL Injection)', (done) => {
    var userRequest = {
...
// Standardized verify reset code request with callback on done()
function testverifyResetPassword(token, userRequest, expectedErrorText, done) {
    chai.request(httpServer)
        .get('/verify_reset_password_code')
        .send(userRequest)
        .end((err, res) => {
